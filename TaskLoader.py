...
class TaskLoader:
    ...
    def load(self, filename):
        ...
        matrix_C = [list(map(float, line.split())) for line in lines[1:N+1]]
        A = list(map(float, lines[N+1].split()))
        B = list(map(float, lines[N+2].split()))

        # Fixing everything
        C = [[float(matrix_C[i][j]) for j in range(M)] for i in range(N)]
        a = A.copy()
        b = B.copy()
        sum_A = sum(a)
        sum_B = sum(b)
